...
import { createTeamHandler, getAllTeamsHandler, getTeamByIdWithFullDetailsForTeamdetailsComponentHandler, updateTeamHandler, deleteTeamHandler, removeMemberFromTeamHandler } from "./teams.controller";

const router = Router();
router.post('/', validateRequest(teamsSchema), createTeamHandler);
router.get('/', getAllTeamsHandler);
router.get('/:id', getTeamByIdWithFullDetailsForTeamdetailsComponentHandler);
