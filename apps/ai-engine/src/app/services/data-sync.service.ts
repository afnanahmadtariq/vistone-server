...
`.trim();
    ...
    const doc: DocumentToIndex = {
      ...
      metadata: {
        name: org.name,
        slug: org.slug,
        memberCount: memberStats.rows[0]?.count || 0,
        projectCount: projectStats.rows[0]?.total || 0,
        activeProjects: projectStats.rows[0]?.active || 0,
        taskCount: taskStats.rows[0]?.total || 0,
        overdueTasks: taskStats.rows[0]?.overdue || 0,
        teamCount: teamStats.rows[0]?.count || 0,
        clientCount: clientStats.rows[0]?.count || 0,
      },
    };
